 ...
 *
 * Same as kthread_run(), but with the kthread bound to prime CPU.
 */
#define kthread_run_perf_critical_prime(threadfn, data, namefmt, ...)		   \
({									   \
	struct task_struct *__k						   \
		= kthread_create(threadfn, data, namefmt, ## __VA_ARGS__); \
	if (!IS_ERR(__k)) {						   \
		__k->flags |= PF_PRIME_PERF_CRITICAL;				   \
		kthread_bind_mask(__k, cpu_gold_mask);			   \
		wake_up_process(__k);					   \
	}								   \
	__k;								   \
})
